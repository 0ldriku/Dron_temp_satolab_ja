...
ただし、日本語で「本の一部でそれ自身に表題があるもの」（英文文献の場合incollection）を引用する場合は、\textit{\textbf{incollectionja}}を使ってください。
...
\subsection{2. Hands \& Gestures}
\begin{tabular}{cll}
\toprule
\textbf{Icon} & \textbf{Command} & \textbf{Description} \\
\midrule
\twemoji{thumbs up} & \verb|\twemoji{thumbs up}| & Thumbs up \\
\twemoji{wave} & \verb|\twemoji{wave}| & Waving hand \\
\twemoji{folded hands} & \verb|\twemoji{folded hands}| & Prayer / Thanks \\
\twemoji{handshake} & \verb|\twemoji{handshake}| & Handshake \\
\bottomrule
\end{tabular}

\subsection{3. Objects \& Tech}
\begin{tabular}{cll}
\toprule
\textbf{Icon} & \textbf{Command} & \textbf{Description} \\
\midrule
\twemoji{rocket} & \verb|\twemoji{rocket}| & Rocket \\
\twemoji{bulb} & \verb|\twemoji{bulb}| & Light bulb (Idea) \\
\twemoji{laptop} & \verb|\twemoji{laptop}| & Laptop computer \\
\twemoji{calendar} & \verb|\twemoji{calendar}| & Calendar \\
\twemoji{party popper} & \verb|\twemoji{party popper}| & Celebration \\
\bottomrule
\end{tabular}
...
吾輩は\twemoji[height=1cm]{cat}\todo{何猫？}である。名前はまだ無い。どこで生まれたかとんと見当がつかぬが、何でも大岡山のキャンパスの片隅で、空腹に耐えかねてニャーニャー泣いていた事だけは記憶している。吾輩はここで初めて「東工大生」というものを見た。彼らは日夜、研究室という名の檻の中でパソコンと格闘している獰猛な種族であるそうだ。

しかし、吾輩の関心は彼らの研究成果にはない。吾輩が今、魂の底から求めているのは、食堂から漂ってくるあの芳しい香り---すなわち「パワー丼」である。

聞くところによれば、その丼は豚肉と玉ねぎを特製のニンニク醤油ダレで炒め、白飯の上に豪快に盛り付けたものだという。人間どもはそれを食らい、午後からの実験に挑む活力を得ている。吾輩もまた、あの一杯を食せば、ただの猫であることを超越し、論文の一本でも書けるようになるのではないかという気がしてならない。

ああ、パワー丼\todo{大中小？}食べたい。腹が減った。食堂のおばさんよ、吾輩の皿にもその茶色い輝きを分けてはくれぬか。